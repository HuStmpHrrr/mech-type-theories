...
Λ-[]′ {_} {σ} {_} {S} {t} {T} (⊨Γ , ⊨Δ , ⊨σ) (∷-cong {i = i} ⊨Δ₁ ⊨S , n , ⊨t) = ⊨Γ , _ , helper
  where
    ...
    ...  | record { ⟦σ⟧ = ⟦σ⟧ ; ⟦δ⟧ = ⟦δ⟧ ; ↘⟦σ⟧ = ↘⟦σ⟧ ; ↘⟦δ⟧ = ↘⟦δ⟧ ; σ≈δ = σ≈δ }
        with ⊨S (⊨-irrel ⊨Δ ⊨Δ₁ σ≈δ)
    ...    | record { ⟦T⟧ = _ ; ⟦T′⟧ = _ ; ↘⟦T⟧ = ↘⟦S⟧ ; ↘⟦T′⟧ = ↘⟦S′⟧ ; T≈T′ = S≈S′ } = record
                          { ⟦T⟧ = _
                          ; ⟦T′⟧ = _
                          ; ↘⟦T⟧ = ⟦[]⟧ ↘⟦σ⟧ (⟦Π⟧ ↘⟦S⟧)
                          ; ↘⟦T′⟧ = ⟦[]⟧ ↘⟦δ⟧ (⟦Π⟧ ↘⟦S′⟧)
                          ; T≈T′ = Π (λ κ → 𝕌-mon κ (𝕌-cumu (m≤m⊔n _ _) S≈S′)) return
                          }
                        , record
                          { ⟦t⟧ = _
                          ; ⟦t′⟧ = _
      ...
      where
        ...
        ...  | record { ⟦T⟧ = _ ; ⟦T′⟧ = _ ; ↘⟦T⟧ = _ ; ↘⟦T′⟧ = _ ; T≈T′ = T≈T′ }
             , record { ⟦t⟧ = _ ; ⟦t′⟧ = _ ; ↘⟦t⟧ = ↘⟦t⟧ ; ↘⟦t′⟧ = ↘⟦t′⟧ ; t≈t′ = t≈t′ } = record
                          ...
                          ; ↘fa = Λ∙ ↘⟦t⟧
                          ; ↘fa′ = Λ∙ (⟦[]⟧ (⟦,⟧ (⟦∘⟧ ⟦wk⟧ ↘⟦δ⟧[κ]) (⟦v⟧ 0)) ↘⟦t′⟧)
                          ; fa≈fa′ = 𝕌-irrel T≈T′ (𝕌-cumu (m≤n⊔m i n) T≈T′) t≈t′
                          }
          where
            ↘⟦δ⟧[κ] : ⟦ σ ⟧s drop (ρ′ [ κ ] ↦ b) ↘ ⟦δ⟧ [ κ ]
            ↘⟦δ⟧[κ] rewrite drop-↦ (ρ′ [ κ ]) b = ⟦⟧s-mon κ ↘⟦δ⟧
  ...
  where
    ...
    ...            | record { ⟦T⟧ = _ ; ⟦T′⟧ = _ ; ↘⟦T⟧ = ↘⟦T⟧ ; ↘⟦T′⟧ = ↘⟦T′⟧ ; T≈T′ = T≈T′ }
                   | record { fa = _ ; fa′ = _ ; ↘fa = ↘fa ; ↘fa′ = ↘fa′ ; fa≈fa′ = fa≈fa′ } = record
                                      ...
                                      ; ↘⟦T⟧ = ⟦[]⟧ (⟦,⟧ ↘⟦σ⟧ (⟦[]⟧ ↘⟦σ⟧ ↘⟦s⟧)) ↘⟦T⟧
                                      ; ↘⟦T′⟧ = ⟦[]⟧ (⟦,⟧ ↘⟦δ⟧ (⟦[]⟧ ↘⟦δ⟧ ↘⟦s′⟧)) ↘⟦T′⟧
                                      ; T≈T′ = 𝕌-cumu (m≤m⊔n n n₁) T≈T′ }
                                    , record
                                      { ⟦t⟧ = _
                                      ; ⟦t′⟧ = _
                                      ; ↘⟦t⟧ = ⟦[]⟧ ↘⟦σ⟧ (⟦$⟧ ↘⟦r⟧ ↘⟦s⟧ ↘fa)
                                      ; ↘⟦t′⟧ = ⟦$⟧ (⟦[]⟧ ↘⟦δ⟧ ↘⟦r′⟧) (⟦[]⟧ ↘⟦δ⟧ ↘⟦s′⟧) ↘fa′
                                      ; t≈t′ = El-cumu (m≤m⊔n n n₁) T≈T′ fa≈fa′
                                      }
