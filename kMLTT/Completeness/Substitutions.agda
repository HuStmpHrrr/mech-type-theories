...
,-∘′ {_} {σ} {_} {T} {t} {_} {τ} (⊨Γ′ , ⊨Γ″ , ⊨σ) ((⊨Γ″₁ , rel) , ⊨T) ((⊨Γ′₁ , ⊨Tσ) , ⊨t) (⊨Γ , ⊨Γ′₂ , ⊨τ) = ⊨Γ , ∷-cong ⊨Γ″ helper , helper″
  where helper : ρ ≈ ρ′ ∈ ⟦ ⊨Γ″ ⟧ρ → RelTyp T ρ T ρ′
        helper = ∷-cong-helper ((⊨Γ″₁ , rel) , ⊨T) ⊨Γ″
        ...
        helper″ {ρ} {ρ′} ρ≈ρ′
          with ⊨τ ρ≈ρ′
        ...  | record { ⟦σ⟧ = ⟦σ⟧ ; ⟦δ⟧ = ⟦δ⟧ ; ↘⟦σ⟧ = ↘⟦σ⟧ ; ↘⟦δ⟧ = ↘⟦δ⟧ ; σ≈δ = σ≈δ ; nat = nat ; nat′ = nat′ }
             with ⊨Tσ (⊨-irrel ⊨Γ′₂ ⊨Γ′₁ σ≈δ) | ⊨t (⊨-irrel ⊨Γ′₂ ⊨Γ′₁ σ≈δ) | ⊨σ (⊨-irrel ⊨Γ′₂ ⊨Γ′ σ≈δ)
        ...     | record { ↘⟦T⟧ = ⟦[]⟧ ↘⟦σ⟧′ ↘⟦T⟧ ; T≈T′ = i , T≈T′ }
                | record { ⟦t⟧ = ⟦t⟧ ; ⟦t′⟧ = ⟦t′⟧ ; ↘⟦t⟧ = ↘⟦t⟧ ; ↘⟦t′⟧ = ↘⟦t′⟧ ; t≈t′ = t≈t′ ; nat = nat₂ ; nat′ = nat′₂ }
                | record { ⟦σ⟧ = ⟦σ⟧₁ ; ⟦δ⟧ = ⟦δ⟧₁ ; ↘⟦σ⟧ = ↘⟦σ⟧₁ ; ↘⟦δ⟧ = ↘⟦δ⟧₁ ; σ≈δ = σ≈δ₁ ; nat = nat₁ ; nat′ = nat′₁ }
                rewrite ⟦⟧s-det ↘⟦σ⟧′ ↘⟦σ⟧₁
