...
®El⇒tm N (t∼a , _ , T≈N)          = conv (®Nat⇒∶Nat t∼a (proj₁ (presup-≈ T≈N))) (≈-sym T≈N)
...
®El-resp-≈ {_} {_} {Γ} (□ A≈B) t∼a t≈t′      = record
  ...
  ; krip = λ {Δ} {σ} Ψs ⊢σ →
    let open □Krip (krip Ψs ⊢σ)
        ...
        ⊢□GT    = proj₁ (proj₂ (proj₂ (presup-≈ (proj₂ T≈))))
        _ , ⊢GT = inv-□-wf ⊢□GT
    in record
    { ua  = ua
    ; ↘ua = ↘ua
    ; rel = ®El-resp-≈ (A≈B (ins (mt σ) (len Ψs))) rel
                       (≈-conv (unbox-cong Ψs (≈-conv ([]-cong t≈t′ (s-≈-refl ⊢σ′)) (≈-trans ([]-cong-Se′ (lift-⊢≈-Se-max (proj₂ T≈)) ⊢σ′) (□-[] ⊢σ′ (lift-⊢-Se-max′ ⊢GT))))
                                           ⊢ΨsΔ refl)
  ...
  ; krip = λ {Δ} {σ} ⊢σ →
    ...
    ; ap-rel = λ s∼b b∈ →
      let open ΛKripke (ap-rel s∼b b∈)
          ⊢σ′     = ⊢r⇒⊢s ⊢σ
          ⊢s      = ®El⇒tm _ s∼b
          ⊢Π      = proj₁ (proj₂ (proj₂ (presup-≈ (proj₂ T≈))))
          j , ⊢IT = inv-Π-wf′ ⊢Π
          k , ⊢OT = inv-Π-wf ⊢Π
      in record
      { fa  = fa
      ...
      ; ®fa = ®El-resp-≈ (ΠRT.T≈T′ (RT (mt σ) b∈)) ®fa
                         (≈-conv ($-cong (≈-conv ([]-cong t≈t′ (s-≈-refl ⊢σ′))
                                                 (≈-trans ([]-cong-Se′ (lift-⊢≈-Se-max (proj₂ T≈)) ⊢σ′)
                                                          (lift-⊢≈-Se-max′ {i = j ⊔ k} {proj₁ T≈} (Π-[] ⊢σ′ (lift-⊢-Se-max ⊢IT) (lift-⊢-Se-max′ ⊢OT)))))
                                         (≈-refl ⊢s))
...
mutual
  ...
  ®-one-sided {Γ = Γ} {T} {i} (ne C≈C′) (ne C≈C″) (⊢T , rel) = ⊢T , helper
    where helper : Δ ⊢r σ ∶ Γ → Δ ⊢ T [ σ ] ≈ Ne⇒Exp (proj₁ (C≈C″ (map len Δ) (mt σ)))
          helper {Δ} {σ} ⊢σ
...
private
  IT-mon-helper : ∀ {i} (iA : ∀ (κ : UMoT) → A [ κ ] ≈ B [ κ ] ∈ 𝕌 i)
                  ...
                  Δ′ ⊢ T [ σ ∘ τ ] ®[ i ] iA (mt (σ ∘ τ)) →
                  Δ′ ⊢ T [ σ ] [ τ ] ®[ i ] iA′ (mt τ)
  IT-mon-helper {σ = σ} {Δ′ = Δ′} {τ} iA RT iA′ RT′ (_ , ⊢T) ⊢σ ⊢τ T∼A = ®-≡ (iA (mt (σ ∘ τ)))
                                                                             (iA′ (mt τ))
                                                                             (®̄-resp-≈ (iA (mt (σ ∘ τ))) T∼A (-, ≈-sym ([∘]-Se ⊢T (⊢r⇒⊢s ⊢σ) (⊢r⇒⊢s ⊢τ))))
...
®-mon : ∀ {i} (A≈B : A ≈ B ∈ 𝕌 i)
        ...
        -----------------------------------
        Δ ⊢ T [ σ ] ®[ i ] A≈Bσ
®-mon {_} {_} {σ} {_} {T} {Δ} {i} (ne {C} C≈C′) (ne C≈C′σ) ((_ , ⊢T) , rel) ⊢σ = (-, t[σ]-Se ⊢T (⊢r⇒⊢s ⊢σ)) , helper
  where helper : Δ′ ⊢r τ ∶ Δ → Δ′ ⊢ sub (sub T σ) τ ≈ Ne⇒Exp (proj₁ (C≈C′σ (map len Δ′) (mt τ)))
        helper {Δ′} {τ} ⊢τ
          with C≈C′σ (map len Δ′) (mt τ) | C≈C′ (map len Δ′) (mt (σ ∘ τ)) | rel (⊢r-∘ ⊢σ ⊢τ)
        ...  | u , ↘u , _ | u′ , ↘u′ , _ | _ , Tστ≈
             rewrite Dn-comp C (mt σ) (mt τ)
                   | Re-det ↘u ↘u′ = -, ≈-trans (lift-⊢≈-Se-max ([∘]-Se ⊢T (⊢r⇒⊢s ⊢σ) (⊢r⇒⊢s ⊢τ))) (lift-⊢≈-Se-max′ Tστ≈)
®-mon N N (_ , T∼A) ⊢σ                                                   = -, ≈-trans ([]-cong-Se′ T∼A (⊢r⇒⊢s ⊢σ)) (N-[] _ (⊢r⇒⊢s ⊢σ))
®-mon (U j<i eq) (U j′<i eq′) (_ , T∼A) ⊢σ                               = -, ≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ T∼A (⊢r⇒⊢s ⊢σ))) (lift-⊢≈-Se-max′ (lift-⊢≈-Se (Se-[] _ (⊢r⇒⊢s ⊢σ)) j<i))
®-mon {□ A} {□ B} {σ} {_} {_} {Δ} {i} (□ A≈B) (□ A≈Bσ) T∼A ⊢σ            = record
  { GT   = GT [ σ ； 1 ]
  ; T≈   = -, ≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ (proj₂ T≈) ⊢σ′)) (lift-⊢≈-Se-max′ (□-[] ⊢σ′ ⊢GT))
  ; krip = helper
  }
  where open Glu□ T∼A
        ⊢σ′ = ⊢r⇒⊢s ⊢σ
        ...
        helper {Δ′} {τ} Ψs ⊢τ = ®-≡ (A≈B (ins (mt σ ø mt τ) (len Ψs)))
          ...
          where open ER
                ⊢τ′   = ⊢r⇒⊢s ⊢τ
                GT[]∼ = krip Ψs (⊢r-∘ ⊢σ ⊢τ)
                ⊢GT[] = proj₂ (®⇒ty (A≈B (ins (mt (σ ∘ τ)) (len Ψs))) GT[]∼)
                ⊢ΨsΔ′ = proj₁ (presup-tm ⊢GT[])
®-mon {Π A _ ρ} {_} {σ} {_} {_} {Δ} {i} (Π iA RT) (Π iA′ RT′) T∼A ⊢σ     = record
  { IT   = IT [ σ ]
  ; OT   = OT [ q σ ]
  ; ⊢OT  = -, t[σ]-Se (proj₂ ⊢OT) (⊢q ⊢σ′ ⊢IT)
  ; T≈   = -, ≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ (proj₂ T≈) (⊢r⇒⊢s ⊢σ))) (lift-⊢≈-Se-max′ {j = proj₁ T≈} (Π-[] (⊢r⇒⊢s ⊢σ) (lift-⊢-Se-max ⊢IT) (lift-⊢-Se-max′ (proj₂ ⊢OT))))
  ; krip = λ ⊢τ → record
  ...
  where open GluΠ T∼A
        ...
        ...     | record { ⟦T⟧ = ⟦T⟧ ; ↘⟦T⟧ = ↘⟦T⟧ ; T≈T′ = T≈T′ }
                ...
                rewrite ρ-comp ρ (mt σ) (mt τ)
                      | ⟦⟧-det ↘⟦T⟧′ ↘⟦T⟧ = ®̄-resp-≈ T≈T′₁ (®-≡ T≈T′ T≈T′₁ rel refl) (-, []-q-∘-, (proj₂ ⊢OT) ⊢σ′ ⊢τ′ ⊢s)
          where open ER
                ⊢τ′  = ⊢r⇒⊢s ⊢τ
                ⊢s   = ®El⇒tm (iA′ (mt τ)) s∼a
  ...
  where ⊢σ′ = ⊢r⇒⊢s ⊢σ
        ...
        helper {Δ′} {τ} ⊢τ
          with C≈C′ (map len Δ′) (mt σ ø mt τ) | C≈C′σ (map len Δ′) (mt τ)
             ...
             | rel (⊢r-∘ ⊢σ ⊢τ)
        ...  | V , ↘V , _ | V′ , ↘V′ , _  | u , ↘u , _ | u′ , ↘u′ , _ | (_ , Tστ≈) , tστ≈
             rewrite Dn-comp C (mt σ) (mt τ)
                   | Dn-comp c (mt σ) (mt τ)
                   | Re-det ↘V ↘V′
                   | Re-det ↘u ↘u′ = (-, ≈-trans (lift-⊢≈-Se-max ([∘]-Se ⊢T ⊢σ′ (⊢r⇒⊢s ⊢τ))) (lift-⊢≈-Se-max′ Tστ≈))
                                   , ≈-conv (≈-trans (≈-sym ([∘] (⊢r⇒⊢s ⊢τ) ⊢σ′ ⊢t)) tστ≈) (≈-sym ([∘]-Se ⊢T ⊢σ′ (⊢r⇒⊢s ⊢τ)))
®El-mon N N (t∼a , _ , T≈N) ⊢σ                                           = ®Nat-mon t∼a ⊢σ , -, ≈-trans ([]-cong-Se′ T≈N (⊢r⇒⊢s ⊢σ)) (N-[] _ (⊢r⇒⊢s ⊢σ))
®El-mon {_} {_} {σ} (U j<i eq) (U j′<i eq′) t∼a ⊢σ
  rewrite Glu-wellfounded-≡ j<i
        | Glu-wellfounded-≡ j′<i                                         = record
  { t∶T = t[σ] t∶T (⊢r⇒⊢s ⊢σ)
  ; T≈  = -, ≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ (proj₂ T≈) (⊢r⇒⊢s ⊢σ))) (lift-⊢≈-Se-max′ (lift-⊢≈-Se (Se-[] _ (⊢r⇒⊢s ⊢σ)) j<i))
  ; A∈𝕌 = 𝕌-mon (mt σ) A∈𝕌
  ...
  where open Glubox t∼a
        ...
        ...        | res
                   rewrite D-ap-vone ua = ®El-resp-≈ Aστ≈′ (®El-resp-T≈ Aστ≈′ res (-, GTστ；≈))
                                                     (≈-conv (unbox-cong Ψs (≈-conv ([∘] ⊢τ′ ⊢σ′ t∶T) (≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ (proj₂ T≈) ⊢στ)) (lift-⊢≈-Se-max′ (□-[] ⊢στ ⊢GT)))) ⊢ΨsΔ′ refl)
...
®El-mon {Π A _ ρ} {_} {σ} {_} {t} {_} {f} {Δ} {i} (Π iA RT) (Π iA′ RT′) t∼a ⊢σ = record
  ...
  ; IT   = IT [ σ ]
  ; OT   = OT [ q σ ]
  ; ⊢OT  = -, t[σ]-Se (proj₂ ⊢OT) ⊢qσ
  ; T≈   = -, ≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ (proj₂ T≈) ⊢σ′)) (lift-⊢≈-Se-max′ {j = proj₁ T≈} (Π-[] (⊢r⇒⊢s ⊢σ) (lift-⊢-Se-max ⊢IT) (lift-⊢-Se-max′ (proj₂ ⊢OT))))
  ; krip = λ {_} {τ} ⊢τ →
  ...
  where open GluΛ t∼a
        ⊢σ′ = ⊢r⇒⊢s ⊢σ
        ⊢IT = proj₂ (®Π-wf iA RT (®El⇒® (Π iA RT) t∼a))
        ⊢qσ = ⊢q ⊢σ′ ⊢IT
                      ...
                      rewrite ρ-comp ρ (mt σ) (mt τ)
                            ...
                            | ⟦⟧-det ↘⟦T⟧′ ↘⟦T⟧ = fa , ↘fa
                                                , ®El-one-sided T≈T′ T≈T′₁
                                                 (®El-resp-≈ T≈T′
                                                 (®El-resp-T≈ T≈T′ ®fa (-, OT,≈))
                                                 (≈-conv ($-cong (≈-conv ([∘] ⊢τ′ ⊢σ′ t∶T)
                                                                         (≈-trans (lift-⊢≈-Se-max ([]-cong-Se′ (proj₂ T≈) ⊢στ))
                                                                                  (lift-⊢≈-Se-max′ {j = proj₁ T≈} (Π-[] ⊢στ (lift-⊢-Se-max ⊢IT) (lift-⊢-Se-max′ (proj₂ ⊢OT))))))
                                                                 (≈-refl ⊢s′))
                                                         (≈-trans (≈-sym ([]-q-∘-,′ (proj₂ ⊢OT) ⊢στ ⊢s′)) OT,≈)))
          where ⊢τ′  = ⊢r⇒⊢s ⊢τ
                ⊢s   = ®El⇒tm Aστ≈′ s∼a
                ⊢s′  = ®El⇒tm Aστ≈ s∼a′
                ⊢στ  = s-∘ ⊢τ′ ⊢σ′
...
mutual
  ...
  ®-cumu-step (ne C≈C′) ((_ , ⊢T) , rel) = (-, cumu ⊢T) , λ ⊢σ → -, ≈-cumu (proj₂ (rel ⊢σ))
  ®-cumu-step N (_ , T∼A)                = -, ≈-cumu T∼A
  ®-cumu-step (U′ j<i) (_ , T∼A)         = -, ≈-cumu T∼A
  ...
  ®-lower : ∀ {i} (A≈B : A ≈ B ∈ 𝕌 i) →
            ...
            -----------------------------
            Γ ⊢ T ®[ i ] A≈B
  ®-lower (ne C≈C′) ((_ , ⊢T) , rel) = (-, cumu ⊢T) , λ ⊢σ → -, ≈-cumu (proj₂ (rel ⊢σ))
  ®-lower N (_ , T∼A)                = -, ≈-cumu T∼A
  ®-lower (U′ j<i) (_ , T∼A)         = -, ≈-cumu T∼A
  ®-lower (□ A≈B) T∼A                = record
    { GT   = GT
    ; T≈   = -, ≈-cumu (proj₂ T≈)
    ; krip = λ {_} {σ} Ψs ⊢σ → ®-lower (A≈B (ins (mt σ) (len Ψs))) (krip Ψs ⊢σ)
    }
    where open Glu□ T∼A
  ®-lower (Π iA RT) T∼A              = record
    { IT   = IT
    ; OT   = OT
    ; ⊢OT  = ⊢OT
    ; T≈   = T≈
    ; krip = λ {_} {σ} ⊢σ →
      let open ΠRel (krip ⊢σ)
      in record
      { IT-rel = ®-lower (iA (mt σ)) IT-rel
      ; OT-rel = λ s∼a a∈ →
        let RT₁  = RT (mt σ) (El-lower _ (iA (mt σ)) (El-cumu-step _ (iA (mt σ)) a∈))
            RT₂  = RT (mt σ) a∈
            T≈T′ = ΠRT.T≈T′ RT₁
        in ®-≡ T≈T′ (ΠRT.T≈T′ RT₂) (®-lower T≈T′ (OT-rel (®El-cumu-step (iA (mt σ)) s∼a) (El-cumu-step _ (iA (mt σ)) a∈))) (⟦⟧-det (ΠRT.↘⟦T⟧ RT₁) (ΠRT.↘⟦T⟧ RT₂))
      }
    }
    where open GluΠ T∼A
  ...
  ®El-lower (□ A≈B) t∼a                    = record
    ...
    ; krip = λ {_} {σ} Ψs ⊢σ →
      ...
      { ua  = ua
      ; ↘ua = ↘ua
      ; rel = ®El-lower (A≈B (ins (mt σ) (len Ψs))) rel
      }
    }
    where open Glubox t∼a
  ®El-lower (Π iA RT) t∼a                  = record
    ...
    ; krip = λ {_} {σ} ⊢σ →
      ...
      { IT-rel = ®-lower (iA (mt σ)) IT-rel
      ; ap-rel = λ s∼b b∈ →
        let open ΛKripke (ap-rel (®El-cumu-step (iA (mt σ)) s∼b) (El-cumu-step _ (iA (mt σ)) b∈))
        in record
        { fa  = fa
        ; ↘fa = ↘fa
        ; ®fa = let RT₁  = RT (mt σ) (El-lower _ (iA (mt σ)) (El-cumu-step _ (iA (mt σ)) b∈))
                    RT₂  = RT (mt σ) b∈
                    T≈T′ = ΠRT.T≈T′ RT₁
                in ®El-≡ T≈T′ (ΠRT.T≈T′ RT₂) (®El-lower T≈T′ ®fa) (⟦⟧-det (ΠRT.↘⟦T⟧ RT₁) (ΠRT.↘⟦T⟧ RT₂))
        }
      }
    }
    where open GluΛ t∼a
  ...
  ®El-cumu-step (Π iA RT) t∼a                = record
    ...
    ; krip = λ {_} {σ} ⊢σ →
      ...
      { IT-rel = ®-cumu-step (iA (mt σ)) IT-rel
      ; ap-rel = λ s∼b b∈ →
        let open ΛKripke (ap-rel (®El-lower (iA (mt σ)) s∼b) (El-lower _ (iA (mt σ)) b∈))
        in record
        { fa  = fa
        ; ↘fa = ↘fa
