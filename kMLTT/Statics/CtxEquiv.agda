...
mutual
  ...
  ctxeq-tm Γ≈Δ (Π-wf ⊢S ⊢T)         = Π-wf (ctxeq-tm Γ≈Δ ⊢S) (ctxeq-tm (∷-cong Γ≈Δ ⊢S (ctxeq-tm Γ≈Δ ⊢S) (≈-refl ⊢S)) ⊢T)
  ctxeq-tm Γ≈Δ (□-wf ⊢T)            = □-wf (ctxeq-tm (κ-cong Γ≈Δ) ⊢T)
  ctxeq-tm Γ≈Δ (vlookup ⊢Γ T∈Γ)
    with ∈!⇒ty≈ ⊢Γ Γ≈Δ T∈Γ      -- TODO: this helper won't work. need to do an inner induction on Γ≈Δ
  ...  | _ , T′∈Δ , _ , T≈T′        = conv (vlookup (proj₂ (presup-⊢≈ Γ≈Δ)) T′∈Δ) {!T≈T′!}
  ...
  ...  | ⊢Γ , ⊢Δ                    = N-E (ctxeq-tm NΓ≈NΔ ⊢T)
                                          (ctxeq-tm Γ≈Δ ⊢s)
                                          (ctxeq-tm (∷-cong NΓ≈NΔ ⊢T (ctxeq-tm NΓ≈NΔ ⊢T) (≈-refl ⊢T)) ⊢r)
                                          (ctxeq-tm Γ≈Δ ⊢t)
    where NΓ≈NΔ = ∷-cong Γ≈Δ (N-wf 0 ⊢Γ) (N-wf 0 ⊢Δ) (N-≈ 0 ⊢Γ)
  ctxeq-tm Γ≈Δ (Λ-I ⊢S ⊢t)          = Λ-I (ctxeq-tm Γ≈Δ ⊢S) (ctxeq-tm (∷-cong Γ≈Δ ⊢S (ctxeq-tm Γ≈Δ ⊢S) (≈-refl ⊢S)) ⊢t)
  ...
  ctxeq-≈ Γ≈Δ (Π-cong ⊢S S≈S′ T≈T′)    = Π-cong (ctxeq-tm Γ≈Δ ⊢S) (ctxeq-≈ Γ≈Δ S≈S′) (ctxeq-≈ (∷-cong Γ≈Δ ⊢S (ctxeq-tm Γ≈Δ ⊢S) (≈-refl ⊢S)) T≈T′)
  ctxeq-≈ Γ≈Δ (□-cong T≈T′)            = □-cong (ctxeq-≈ (κ-cong Γ≈Δ) T≈T′)
  ctxeq-≈ Γ≈Δ (v-≈ ⊢Γ T∈Γ)             = {!!} -- TODO: see above
  ...
  ...  | ⊢Γ , ⊢Δ                       = rec-cong (ctxeq-tm NΓ≈NΔ ⊢T)
                                                  (ctxeq-≈ NΓ≈NΔ T≈T′)
                                                  (ctxeq-≈ Γ≈Δ s≈s′)
                                                  (ctxeq-≈ (∷-cong NΓ≈NΔ ⊢T (ctxeq-tm NΓ≈NΔ ⊢T) (≈-refl ⊢T)) r≈r′)
                                                  (ctxeq-≈ Γ≈Δ t≈t′)
    where NΓ≈NΔ = ∷-cong Γ≈Δ (N-wf 0 ⊢Γ) (N-wf 0 ⊢Δ) (N-≈ 0 ⊢Γ)
  ctxeq-≈ Γ≈Δ (Λ-cong ⊢S t≈t′)         = Λ-cong (ctxeq-tm Γ≈Δ ⊢S) (ctxeq-≈ (∷-cong Γ≈Δ ⊢S (ctxeq-tm Γ≈Δ ⊢S) (≈-refl ⊢S)) t≈t′)
  ...
  ...  | ⊢Γ , ⊢Δ                       = rec-β-su (ctxeq-tm NΓ≈NΔ ⊢T)
                                                  ...
                                                  (ctxeq-tm Γ≈Δ ⊢t)
    where NΓ≈NΔ = ∷-cong Γ≈Δ (N-wf 0 ⊢Γ) (N-wf 0 ⊢Δ) (N-≈ 0 ⊢Γ)
  ctxeq-≈ Γ≈Δ (Λ-β ⊢S ⊢t ⊢s)           = Λ-β (ctxeq-tm Γ≈Δ ⊢S) (ctxeq-tm (∷-cong Γ≈Δ ⊢S (ctxeq-tm Γ≈Δ ⊢S) (≈-refl ⊢S)) ⊢t) (ctxeq-tm Γ≈Δ ⊢s)
  ...
  ctxeq-s : ⊢ Γ ≈ Δ →
            ...
            -----------
            Δ ⊢s σ ∶ Γ′
  ctxeq-s Γ≈Δ (s-I _)            = s-conv (s-I (proj₂ (presup-⊢≈ Γ≈Δ))) {!!}
  ctxeq-s Γ≈Δ (s-p ⊢σ)           = {!!}
  ctxeq-s Γ≈Δ (s-∘ ⊢σ ⊢δ)        = {!!}
  ctxeq-s Γ≈Δ (s-, ⊢σ ⊢T ⊢t)     = {!!}
  ctxeq-s Γ≈Δ (s-； Ψs ⊢σ ⊢Γ eq) = {!!}
  ctxeq-s Γ≈Δ (s-conv ⊢σ Γ″≈Γ′)  = {!!}
