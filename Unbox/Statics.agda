...
-- Relab Δ m n Δ′ meaning weakening (m - 1) contexts before n'th, generating Δ′
data Relab : Envs → ℕ → ℕ → Envs → Set where
  fuse : Relab (Γ′ ∷ Γ ∷ Δ) 0 0 ((Γ′ ++ Γ) ∷ Δ)
  init : ∀ m → L.length Δ′ ≡ m → Relab (Γ′ ∷ Γ ∷ Δ) (suc m) 0 ((Γ′ ++ Γ″) ∷ Δ′ ++ Γ ∷ Δ)
  cons : ∀ {m n} → Relab Δ m n Δ′ → Relab (Γ ∷ Δ) m (suc n) (Γ ∷ Δ′)

relab : Exp → ℕ → ℕ → ℕ → Exp
relab (v x) m n k                            = v x
relab * m n k                                = *
relab ze m n k                               = ze
relab (su t) m n k                           = su (relab t m n k)
relab (Λ t) m 0 k                            = Λ (relab t m 0 (k + 1))
relab (Λ t) m (suc n) k                      = Λ (relab t m (suc n) k)
relab (s $ t) m n k                          = relab s m n k $ relab t m n k
relab (box t) m n k                          = box (relab t m (suc n) k)
relab (unbox x t) m n k with x Nₚ.≤?> n
... | tri< a ¬b ¬c                           = unbox x (relab t m (n ∸ x) k)
relab (unbox x t) 0 n k       | tri≈ ¬a b ¬c = unbox n (t ↑[ k ] 0)
relab (unbox x t) (suc m) n k | tri≈ ¬a b ¬c = unbox (x + m) t
... | tri> ¬a ¬b c                           = unbox (x + (m ∸ 1)) t
...
-- module SyntacticProperties where
...
--   auto-weaken-hd : Γ ∷ Δ′ ⊢ t ∶ T →
--                    (Γ ++ Γ′) ∷ Δ′ ⊢ t ∶ T
--   auto-weaken-hd = auto-weaken []
...
-- open SyntacticProperties public
