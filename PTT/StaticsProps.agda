...
mutual
  ...
  vlookup-≈ : ∀ {x i} Δ →
              ...
              Γ′ ≡ Δ ++ S ∷ Γ →
              Δ ++ S′ ∷ Γ ⊢ v x ∶ T
  vlookup-≈ [] (⊢∷ ⊢Γ′ ⊢S) S≈S′ ⊢S′ here refl              = conv (vlookup ⊢S′Γ here)
                                                                  (≈-≲ (≈-conv ([]-cong (↑-≈ ⊢S′Γ) (≈-sym S≈S′)) (≈-≲ (Se-[] (S-↑ ⊢S′Γ) ℕₚ.≤-refl))))
    where ⊢S′Γ = ⊢∷ ⊢Γ′ ⊢S′
  ...
  ty-env-subst S≈S′ ⊢S′ (conv ⊢t T≲T′) refl              = conv (ty-env-subst S≈S′ ⊢S′ ⊢t refl) {!!}
  ...
  subst-env-subst (U ∷ Δ) S≈S′ ⊢S′ (S-↑ (⊢∷ ⊢SΔ′ ⊢U)) refl = S-conv {!!} (S-↑ (⊢∷ {!!} {!!}))
  subst-env-subst Δ S≈S′ ⊢S′ (S-I ⊢Δ′) eq           = {!!}
  subst-env-subst Δ S≈S′ ⊢S′ (S-∘ ⊢σ ⊢σ₁) eq        = {!!}
  subst-env-subst Δ S≈S′ ⊢S′ (S-, ⊢σ x x₁) eq       = {!!}
  subst-env-subst Δ S≈S′ ⊢S′ (S-conv x r) eq = {!!}
