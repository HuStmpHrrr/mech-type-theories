...
module NonCumulative.Ascribed.Soundness.Properties.Bundle (fext : ∀ {ℓ₁ ℓ₂} → Extensionality ℓ₁ ℓ₂) where
...
®-Π-𝕌 : ∀ {i j k} →
  ...
  Γ ⊢ T ®[ i ] iA
®-Π-𝕌 {j = j} {k = k} refl jA RT (Π i≡maxjk jA′ RT′ _ _) T®
...
®-L-𝕌 : ∀ {i j k} →
  ...
  Γ ⊢ T ®[ i ] iA
®-L-𝕌 {j = j} {k = k} kA (L i≡j+k kA′ _ _) T®
...
®El-Π-𝕌 : ∀ {i j k} →
  ...
  Γ ⊢ t ∶ T ®[ i ] a ∈El iA
®El-Π-𝕌 {j = j} {k = k} refl jA RT (Π i≡maxjk jA′ RT′ _ _) t®
  rewrite 𝕌-wf-gen j (ΠI≤ i≡maxjk)
        | 𝕌-wf-gen k (ΠO≤ i≡maxjk)
        | Glu-wf-gen j (ΠI≤ i≡maxjk)
        | Glu-wf-gen k (ΠO≤ i≡maxjk) = record
