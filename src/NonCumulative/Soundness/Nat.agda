...
N-wf′ ⊩Γ = record
    { ⊩Γ = ⊩Γ ; krip = helper }
    where helper : Δ ⊢s σ ∶ ⊩Γ ® ρ → GluExp 1 Δ N (Se 0) σ ρ
          helper σ∼ρ = record {
            ⟦T⟧ = U 0 ;
            ⟦t⟧ = N ;
            ↘⟦T⟧ = ⟦Se⟧ 0 ;
            ↘⟦t⟧ = ⟦N⟧ ;
            T∈𝕌 = U′ ;
            t∼⟦t⟧ = record { t∶T = t[σ] (N-wf (⊩⇒⊢ ⊩Γ)) ⊢σ ; T≈ = Se-[] 0 ⊢σ ; A∈𝕌 = N refl ; rel = N-[] ⊢σ }
            }
            where ⊢σ = s®⇒⊢s ⊩Γ σ∼ρ
...
cons-N : (⊩NΓ : ⊩ (N ↙ 0) ∷ Γ) → cons-N-type ⊩NΓ
...
N-E-helper-type : ∀ {i} → ⊩ (T ↙ i) ∷ (N ↙ 0) ∷ Γ → Set
N-E-helper-type {T} {Γ} ⊩TNΓ@(⊩∷ {i = i} ⊩NΓ@(⊩∷ ⊩Γ _ _) _ gluT) =
  ∀ {Δ s r σ ρ t a} →
  (N ↙ 0) ∷ Γ ⊢ T ∶[ 1 + i ] Se i →
  Γ ⊢ s ∶[ i ] T [| ze ∶ N ↙ 0 ] →
  (T ↙ i) ∷ (N ↙ 0) ∷ Γ ⊢ r ∶[ i ] T [ (wk ∘ wk) , su (v 1) ∶ N ↙ 0 ] →
  (σ®ρ : Δ ⊢s σ ∶ ⊩Γ ® ρ) →
  (glus : GluExp i Δ s (T [| ze ∶ N ↙ 0 ]) σ ρ) →
  (∀ {Δ σ ρ} → Δ ⊢s σ ∶ ⊩TNΓ ® ρ → GluExp i Δ r (T [ (wk ∘ wk) , su (v 1) ∶ N ↙ 0 ]) σ ρ) →
  (t®a : Δ ⊢ t ∶N® a ∈Nat) →
  let open GluExp glus hiding (T∈𝕌)
      open GluTyp (gluT (cons-N ⊩NΓ σ®ρ t®a))
  in ∃ λ ra → rec∙ (T ↙ i) , ⟦t⟧ , r , ρ , a ↘ ra
               × Δ ⊢ rec (T [ q ( N ↙ 0 ) σ ] ↙ i) (s [ σ ]) (r [ q ( T ↙ i ) (q ( N ↙ 0 ) σ) ]) t ∶ T [ σ , t ∶ N ↙ 0 ] ®[ i ] ra ∈El T∈𝕌

N-E-helper : ∀ {i} → (⊩TNΓ : ⊩ (T ↙ i) ∷ (N ↙ 0) ∷ Γ) →
             N-E-helper-type ⊩TNΓ
N-E-helper {T} {Γ} ⊩TNΓ@(⊩∷ {i = i} ⊩NΓ@(⊩∷ ⊩Γ _ _) _ gluT′) {Δ} {s} {r} {σ} {ρ} {_} {_}
  ...
  where
    open ER
    rec′ : Exp → Exp
    rec′ t = rec (T [ q ( N ↙ 0 ) σ ] ↙ i) (s [ σ ]) (r [ q ( T ↙ i ) (q ( N ↙ 0 ) σ) ]) t
    ⊢σ   = s®⇒⊢s ⊩Γ σ®ρ
    open NatTyping ⊢T ⊢σ
    ≈N   = ≈-sym (N-[] ⊢σ)
    ⊢ze′ = conv (ze-I ⊢Δ) ≈N

    gluT : Δ ⊢ t ∶N® a ∈Nat → GluTyp i Δ T (σ , t ∶ N ↙ 0) (ρ ↦ a)
    gluT t®a′ = gluT′ (cons-N ⊩NΓ σ®ρ t®a′)

    glur : (t®a : Δ ⊢ t ∶N® a ∈Nat) →
           (let open GluTyp (gluT t®a) renaming (T∈𝕌 to T∈𝕌′) in Δ ⊢ t′ ∶ T [ σ , t ∶ N ↙ 0 ] ®[ i ] b ∈El T∈𝕌′) →
           GluExp i Δ r (T [ (wk ∘ wk) , su (v 1) ∶ N ↙ 0  ]) ((σ , t ∶ N ↙ 0) , t′ ∶ T ↙ i) (ρ ↦ a ↦ b)
    glur t®a t′®b = glur′ (s®-cons ⊩TNΓ (cons-N ⊩NΓ σ®ρ t®a) t′®b)

    T[σ,ze]≈T[|ze][σ] : Δ ⊢ T [ σ , ze ∶ N ↙ 0 ] ≈ T [| ze ∶ N ↙ 0 ] [ σ ] ∶[ 1 + i ] Se i
    T[σ,ze]≈T[|ze][σ] = ≈-sym ([]-I,ze-∘ ⊢T ⊢σ)

    T[σ,t]≈T[qσ][|t]′ : Δ ⊢ t ∶[ 0 ] N → Δ ⊢ T [ σ , t ∶ N₀ ] ≈ T [ q N₀ σ ] [| t ∶ N [ σ ] ↙ 0 ] ∶[ 1 + i ] Se i
    T[σ,t]≈T[qσ][|t]′ ⊢t = []-q-∘-,′ ⊢T ⊢σ (conv ⊢t ≈N)

    T[σ,t]≈T[qσ][|t] : Δ ⊢ t ∶[ 0 ] N → Δ ⊢ T [ σ , t ∶ N₀ ] ≈ T [ q N₀ σ ] [| t ∶ N ↙ 0 ] ∶[ 1 + i ] Se i
    T[σ,t]≈T[qσ][|t] ⊢t = ≈-trans (T[σ,t]≈T[qσ][|t]′ ⊢t) ([]-cong-Se‴ ⊢Tqσ
                                                                      (s-≈-sym (,-cong (I-≈ ⊢Δ) Δ⊢N ≈N
                                                                               (≈-conv-N-[]-sym (≈-refl ⊢t) (s-I ⊢Δ)))))

    T[σ,ze]≈T[qσ][|ze] : Δ ⊢ T [ σ , ze ∶ N₀ ] ≈ T [ q N₀ σ ] [| ze ∶ N ↙ 0 ] ∶[ 1 + i ] Se i
    ...
    ...    | record { ⟦T⟧ = ⟦T⟧₁ ; ↘⟦T⟧ = ↘⟦T⟧₁ ; T∈𝕌 = T∈𝕌₁ ; T∼⟦T⟧ = T∼⟦T⟧ }
      ...
      where
        ...
        qσ∘[|t'],rec′t′≈σ,t′,rec′t′ = s-≈-sym (,-cong (s-≈-sym (s-≈-trans (∘-cong (,-cong (I-≈ ⊢Δ) Δ⊢N ≈N (≈-refl (conv ⊢t′ (≈-sym (N-[] (s-I ⊢Δ)))))) (s-≈-refl ⊢qσ)) (qσ∘[I,t]≈σ,t ⊢Δ Γ⊢N ⊢σ ⊢t′₁))) ⊢T (≈-refl ⊢T) (≈-refl ⊢wect′))

        r[σ,t′,rec′t′]≈rec′t : Δ ⊢ r [ (σ , t′ ∶ N ↙ 0) , rec′ t′ ∶ T ↙ i ] ≈ rec′ t ∶[ i ] T [ σ , t ∶ N ↙ 0 ]
        r[σ,t′,rec′t′]≈rec′t =
          begin
            r [ (σ , t′ ∶ N ↙ 0) , rec′ t′ ∶ T ↙ i ] ≈⟨ ≈-conv ([]-cong (≈-refl ⊢w) (s-≈-sym qσ∘[|t'],rec′t′≈σ,t′,rec′t′)) T[wkwk,suv1][σ,t′,rec′t′]≈T[σ,t] ⟩
            r [ (q N₀ σ ∘ I , t′ ∶ N₀) , rec′ t′ ∶ T ↙ i ] ≈˘⟨ ≈-conv ([]-q-, ⊢qσ ⊢T ⊢I,t′ (N-E′ ⊢t′) ⊢w)
                                                                       (≈-trans ([]-cong-Se‴ (t[σ]-Se ⊢T (s-, ⊢wkwk Γ⊢N ⊢suv1)) qσ∘[|t'],rec′t′≈σ,t′,rec′t′) T[wkwk,suv1][σ,t′,rec′t′]≈T[σ,t]) ⟩
      ...
      where
        ...
        ...     | A , W , ↘A , ↘W
          ...
          where
            ...
            eq₉ : (T [ q N₀ σ ] [ q N₀ τ ] ↙ i) ∷ N₀ ∷ Δ′ ⊢
                  T [ (wk ∘ wk) , su (v 1) ∶ N₀ ] [ q (T ↙ i) (q N₀ σ) ∘ q ((T [ q N₀ σ ]) ↙ i) (q N₀ τ) ]
                  ≈ T [ q N₀ σ ] [ q N₀ τ ] [ (wk ∘ wk) , su (v 1) ∶ N₀ ] ∶[ 1 + i ] Se i
            eq₉ = begin
            ...
            qqσqqτ∼ρτll : ((T [ q N₀ σ ∘ q N₀ τ ]) ↙ i) ∷ N₀ ∷ Δ′ ⊢s q (T ↙ i) (q N₀ σ) ∘ q ((T [ q N₀ σ ]) ↙ i) (q N₀ τ) ∶ ⊩TNΓ
                ® ρ ↦ l′ 0 N (len Δ′) ↦ l′ i (GluTyp.⟦T⟧ (gluT′ qσqτ∼ρτl)) (1 + (len Δ′))
            qqσqqτ∼ρτll
            ...
            T[qσ][τ,t[τ]]≈T[qσ][qτ][|t[τ]] = begin
                ...
                T [ q N₀ σ ] [ τ , t [ τ ] ∶ N ↙ 0 ] ≈⟨ []-q-∘-,′ ⊢Tqσ ⊢τ′ (t[σ] ⊢t ⊢τ′) ⟩
            ...
            ...              | _ , ↘B , _ | eqT
                       ...
                       = ≈-conv ( begin
                        ...
                        rec (T [ q N₀ σ ] [ q N₀ τ ] ↙ i) (s [ σ ] [ τ ]) (r [ q (T ↙ i) (q N₀ σ) ] [ q ((T [ q N₀ σ ]) ↙ i) (q N₀ τ) ]) (t [ τ ]) ≈⟨
                              ≈-conv (rec-cong ⊢Tqσqτ
                                               (≈-trans T[qσ][qτ]≈T[qσ∘qτ] eqT)
                                               (≈-conv eqs T[|ze][σ][τ]≈T[qσ][qτ][|ze])
                                               (≈-conv (≈-trans (≈-sym ([∘] ⊢qqτ ⊢qqσ ⊢w)) (ctxeq-≈ (⊢≈-sym TqqNΔ′≈) eqr)) eq₉)
                                               eqc)
