...
module NonCumulative.Statics.Unascribed.Full.Misc where
...
t[σ]-Se : Δ ⊢ T ∶ Se i → Γ ⊢s σ ∶ Δ → Γ ⊢ T [ σ ] ∶[ 1 + i ] Se i
...
⊢q : ∀ {i} → ⊢ Γ → Γ ⊢s σ ∶ Δ → Δ ⊢ T ∶[ 1 + i ] Se i → (T [ σ ]) ∷ Γ ⊢s q σ ∶ T ∷ Δ
...
⊢I,t : ∀ {i} → ⊢ Γ → Γ ⊢ T ∶[ 1 + i ] Se i → Γ ⊢ t ∶[ i ] T → Γ ⊢s I , t ∶ T ∷ Γ
