...
module NonCumulative.Statics.Ascribed.Refl where
...
mutual
  ...
  ≈-refl′ (Λ-I ⊢S ⊢t eq)       = Λ-cong ⊢S (≈-refl′ ⊢S) (≈-refl′ ⊢t) eq
...
∷-cong′ : ∀ {i} → ⊢ Γ → Γ ⊢ T ∶[ 1 + i ] Se i → Γ ⊢ T′ ∶[ 1 + i ] Se i → Γ ⊢ T ≈ T′ ∶[ 1 + i ] Se i → ⊢ (T ↙ i) ∷ Γ ≈ (T′ ↙ i) ∷ Γ
