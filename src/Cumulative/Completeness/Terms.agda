...
module Cumulative.Completeness.Terms (fext : Extensionality 0ℓ (suc 0ℓ)) where
...
⊨-lookup-gen {T = sub T _} {sub T′ _} {suc x} (∷-cong Γ≈Δ rel) (there T∈Γ) (there T′∈Δ)
  ...
  where helper : ρ ≈ ρ′ ∈ ⟦ ∷-cong Γ≈Δ rel ⟧ρ →
                 ...
                 λ rel → RelExp (v (1 + x)) ρ (v (1 + x)) ρ′ (El _ (RelTyp.T≈T′ rel))
        helper (ρ≈ρ′ , _)
          with f ρ≈ρ′
...
[,]-v-su′ : ∀ {i x} →
            ...
            x ∶ T ∈! Δ →
            ---------------------------------------------
            Γ ⊨ v (1 + x) [ σ , s ] ≈ v x [ σ ] ∶ T [ σ ]
[,]-v-su′ {_} {σ} {_} {S} {s} {T} {_} {x} (⊨Γ , ⊨Δ , ⊨σ) (⊨Δ₁ , _ , ⊨S) (⊨Γ₁ , _ , ⊨s) T∈Δ = ⊨Γ , _ , helper
  where helper : ρ ≈ ρ′ ∈ ⟦ ⊨Γ ⟧ρ →
                 --------------------------------------------------------------------------------
                 Σ (RelTyp _ (T [ σ ]) ρ (T [ σ ]) ρ′)
                 λ rel → RelExp (v (1 + x) [ σ , s ]) ρ (v x [ σ ]) ρ′ (El _ (RelTyp.T≈T′ rel))
        helper {ρ} {ρ′} ρ≈ρ′ = help
          where module σ = RelSubst (⊨σ ρ≈ρ′)
